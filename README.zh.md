...
| 加载更早历史 | 导航条最上方的 ↑ 按钮（仅当会话还有更早历史时显示）：点击加载更早的消息——转发给官方加载更早控件，保持当前阅读位置不跳动；悬停立即显示与节点预览同款的提示卡（官方 HoverCard 视觉） |
...
# 或 npm 源：dsh plugin --profile web add @vlln/dsh-navbar@0.4.0
```
...
**加载更早历史**：当前会话还有更早消息（历史窗口未加载完）时，导航条最上方出现 ↑ 按钮。点击加载更早的消息——导航条顶部随之新增节点。全部历史加载完后按钮自动隐藏；加载进行中按钮禁用。悬停按钮立即显示与节点预览同款的提示卡（官方 HoverCard 视觉），加载中显示「加载中…」。点击转发给官方加载更早控件，当前阅读位置保持不变（不跳动）。

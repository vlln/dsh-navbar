...
`;
				...
				const findSidebar = () => {
					for (const el of document.querySelectorAll("div")) if (getComputedStyle(el).getPropertyValue("--dsh-sidebar-inline-padding").trim() !== "" && el.offsetWidth > 0) return el;
					return null;
				};
				const sidebarOf = () => {
				...
				const position = () => {
					...
					const nextLeft = `${Math.round(Math.max(anchor + 8, 8))}px`;
				...
				const WINDOW = Number.POSITIVE_INFINITY;
				const HALF_WINDOW = 0;
				let lo = 0;
				let builtRows = [];
				const LEVEL = [
				...
				const levelWidth = (d) => Math.round(peakW * LEVEL[Math.min(d, LEVEL.length - 1)] / LEVEL[0]);
				let hoverIndex = null;
				const applyShape = () => {
					ensureSizes();
				...
				const render = () => {
					...
					if (rows.length === builtRows.length && rows.every((row, i) => row === builtRows[i])) {
						applyShape();
						return;
					}
					bar.textContent = "";
				...
				const onNearMove = (event) => updateNear(event.clientY);
				const onNearLeave = () => {
